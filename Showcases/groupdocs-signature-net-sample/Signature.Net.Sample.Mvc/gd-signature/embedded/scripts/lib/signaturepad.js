...
d3 = function () {
  ...
  if ((/WebKit/.test(navigator.userAgent)) || (/MSIE/.test(navigator.userAgent) && screen.deviceXDPI / screen.logicalXDPI > 1)) {
...
var signaturePad = function signaturePad(containerId, signatureColor) {
    var PAD_WIDTH = 264,
        PAD_HEIGHT = 88,
        PAD_COLOR = "#ffffff",
        PATH_SEGMENT_MAX_POINTS_COUNT = 20000000,//parce  path to multiple paths, does not helps at all 
        SKIP_POINTS = 2,//do not use each mouse move points - make line smooth
        PIXEL_DIFF = 1, // minimumm space between two poins
        MOUSE_SPEED_UPDATE_INTERVAL = 100,
        MOUSE_AVERAGE_POINT_COUNT = 5,// mouse speed sensitivity
        dataContainer = [],
        linevar = null,
        svg = null,
        r = null,
        ...
        lineHeight = function () {
            ...
            var avg = sum / mpoints.length;
            // console.log(avg);
            if (avg == 0) avg = 2;//initial height
            result = 4 - avg;
            if (isNaN(result))
                return 1;
            else if (result < 0.5)
        ...
        down = function () {
            ...
            if (typeof (this.getBoundingClientRect) === 'undefined') {
                rect = $(svg[0][0]).find('rect')[0];
            };
            isDown = true;
            wasOut = false;
            var e = d3.touches(this);
            if (!e || e.length == 0) e = d3.mouse(rect);
            var coor = getCoords(e);
            var dat = [];
            resetSpeed();
            ...
            for (i = 0; i < 1; i++) {
                angle = i;
                x = coor.x + (0.02 * angle) * Math.cos(angle);
                y = coor.y + (0.02 * angle) * Math.sin(angle);

                dat.push({ x: x, y: y, w: 0.9 });
            }
            dataContainer.push(dat);
            addPath();
            d3.event.preventDefault();//FF 
        ...
        move = function () {
            if (isDown) {
                if (skippedPoints == 0) {
                    var e = d3.touches(this);
                    if (!e || e.length == 0) e = d3.mouse(this);
                    var coor = getCoords(e);
                    var lastDatArr = dataContainer[dataContainer.length - 1];
                    var lastDat = lastDatArr[lastDatArr.length - 1];
                    if ((Math.abs(lastDat.x - coor.x) > PIXEL_DIFF) || (Math.abs(lastDat.y - coor.y) > PIXEL_DIFF)) {
                        dataContainer[dataContainer.length - 1].push({ x: coor.x, y: coor.y, w: lineHeight() });
                        //  console.log("pushed " + JSON.stringify(coor));
                    }
                }
                skippedPoints++;
                if (skippedPoints > SKIP_POINTS)
                    skippedPoints = 0;
                doMove();
            }
            d3.event.preventDefault();
        ...
        doMove = function () {
            ...
            if (dataContainer.length > 0 && dataContainer[dataContainer.length - 1].length >= PATH_SEGMENT_MAX_POINTS_COUNT) {

                //break current path and continue with new one
                var lastPos = dataContainer[dataContainer.length - 1];
                var dat = [];
                dat.push(lastPos[lastPos.length - 1]);
                //lastPos.pop();
                dataContainer.push(dat);
                addPath();

            } else {
                svg.selectAll("path").data(dataContainer).attr("d", linevar);
            }
        ...
        addPath = function () {
            svg.selectAll("path").data(dataContainer).enter()
                .append("svg:path")
                .attr("d", linevar)
                .attr("fill", getSignatureColor())
        ...
        resetSpeed = function () {
            ...
            mousetravel = 0;
            mpoints.length = 0;
            setTimeout(calcMouseSpeed, MOUSE_SPEED_UPDATE_INTERVAL);
        },

        calcMouseSpeed = function () {
            if (!isDown)
                return;

            var mousex = currentMouseX;
            var mousey = currentMouseY;
            if (lastmousex > -1) {
            ...
            if (lastmousetime && lastmousetime != timenow) {
                ...
                if (mpoints.length > MOUSE_AVERAGE_POINT_COUNT)
                    mpoints.splice(0, 1);
            }
        ...
        clear = function () {
            dataContainer = [];
            svg.selectAll("path").data(dataContainer).attr("d", linevar).exit().remove();
        },

        undoLast = function () {
            dataContainer = dataContainer.splice(0, dataContainer.length - 1);
            svg.selectAll("path").data(dataContainer).attr("d", linevar).exit().remove();
        },

        getSvg = function () {
            var path = svg.selectAll("path")[0];
            var pathStr = "";
            path.forEach(function(d, i) {
                pathStr += d.getAttribute("d");

            });
            return '<svg xmlns="http://www.w3.org/2000/svg" version="1.1"  viewBox="0 0 ' + PAD_WIDTH + ' ' + PAD_HEIGHT + '" preserveAspectRatio="none" width="100%" height="100%"><path pointer-events="none" fill="' + getSignatureColor() + '" d="' + pathStr + '"></path></svg>';
        },
        getPaths = function () {
            return svg.selectAll("path")[0]; 
        },
        getJson = function () {
            return JSON.stringify(dataContainer);
        },
        getSignatureColor = function() {
            if (typeof(signatureColor) == "function")
        ...
        init = function () {
            ...
            svg = d3.selectAll(containerId + " svg");
            if (svg[0].length == 0) {
                svg = d3.select(containerId)
                    .append("svg:svg")
                    .attr("height", PAD_HEIGHT)
                    .attr("width", PAD_WIDTH)
                    .attr("viewBox", "0 0 " + PAD_WIDTH + " " + PAD_HEIGHT)
                    .attr("class", "pad")
                    .attr({ version: '1.1', xmlns: "http://www.w3.org/2000/svg" });
            }

            r = svg.append("rect:rect")
                 .attr("height", PAD_HEIGHT)
                 .attr("width", PAD_WIDTH)
                 .attr("fill", PAD_COLOR)
                 .attr("class", "pad")
...
// original https://raw.github.com/larskotthoff/d3/varline/src/svg/line-variable.js
...
function d3_svg_lineVariablePoints(self, d, x, y, w, intpol, t) {
    ...
    for (i = 0; i < n; i++) {
        ...
        //if (aleft == 0)
        //{
        //    console.log("fixing aleft");
        //    aleft = 0.00001;
        //}
        //if (aright == 0) {
        //    console.log("fixing aright");
        //    aright = -0.00001;
        //}
        var gamma = aleft - aright;
        //if (gamma == 0) {
        //    console.log("fixing gamma");
        //    gamma = 10000;
        //}
        ...
        //console.log("dxup " + Math.round( dxup*10000)/10000);
        //console.log("dyup " + Math.round(dyup * 10000) / 10000);
        //console.log("dxdown " + Math.round(dxdown * 10000) / 10000);
        //console.log("dydown " + Math.round(dydown * 10000) / 10000);
        //console.log("=================================================" );
        if (!isNaN(dxdown) && !isNaN(dydown) && isFinite(dxdown) && isFinite(dydown)) { //nmm
            if (!(thisx + dxup < 0 || thisy - dyup < 0 || thisx + dxdown < 0 || thisy - dydown < 0)) {
    ...
    tmpLine = line(tmpStack.reverse()),
    line1 = tmpLine ? tmpLine.replace("M", "L") : null;
