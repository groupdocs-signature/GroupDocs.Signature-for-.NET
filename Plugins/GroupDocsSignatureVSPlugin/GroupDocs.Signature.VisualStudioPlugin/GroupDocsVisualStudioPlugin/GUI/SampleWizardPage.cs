...
using System.Xml;
using GroupDocsSignatureVisualStudioPlugin.XML;
...
namespace GroupDocsSignatureVisualStudioPlugin.GUI
{
    public partial class SampleWizardPage : Form
    {
        ...
        bool CopyAndCreateProject()
        {
            ...
            //string sampleSourcePath = "GroupDocs.Signature";
            ...
            try
            {
                ...
                {
                    //For CSharp Project...
                    if (!Directory.Exists(Path.Combine(destinationPath, "CSharp\\GroupDocs.Signature.Examples.CSharp", "Bin", "Debug")))
                        Directory.CreateDirectory(Path.Combine(destinationPath, "CSharp\\GroupDocs.Signature.Examples.CSharp", "Bin", "Debug"));
                    File.Copy(dllsPaths[i], Path.Combine(destinationPath, "CSharp\\GroupDocs.Signature.Examples.CSharp", "Bin", "Debug", Path.GetFileName(dllsPaths[i])), true);

                    //For VBasic Project...
                    if (!Directory.Exists(Path.Combine(destinationPath, "VisualBasic\\GroupDocs.Signature.Examples.VBasic", "Bin", "Debug")))
                        Directory.CreateDirectory(Path.Combine(destinationPath, "VisualBasic\\GroupDocs.Signature.Examples.VBasic", "Bin", "Debug"));
                    File.Copy(dllsPaths[i], Path.Combine(destinationPath, "VisualBasic\\GroupDocs.Signature.Examples.VBasic", "Bin", "Debug", Path.GetFileName(dllsPaths[i])), true);
                }

                progressBar1.Value = 50;

                string[] projectFiles = Directory.GetFiles(Path.Combine(destinationPath, (rdbCSharp.Checked ? "CSharp\\GroupDocs.Signature.Examples.CSharp" : "VBasic\\GroupDocs.Signature.Examples.VBasic")), (rdbCSharp.Checked ? "*.csproj" : "*.vbproj"));
   ...
   TreeNode nodeToAddTo)
        ...
        private void rdbCSharp_CheckedChanged(object sender, EventArgs e)
        {
            ...
            GroupDocsComponents.list.TryGetValue(Constants.GROUPDOCS_COMPONENT, out component);
            string repoPath = GitHelper.getLocalRepositoryPath(component);
            PopulateTreeView(repoPath + "/Examples/" + (rdbCSharp.Checked ? "CSharp\\GroupDocs.Signature.Examples.CSharp" : "VBasic\\GroupDocs.Signature.Examples.VBasic"));
