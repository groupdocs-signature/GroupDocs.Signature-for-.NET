...
using GroupDocsSignatureVisualStudioPlugin.GUI;
using GroupDocsSignatureVisualStudioPlugin.Properties;

namespace GroupDocsSignatureVisualStudioPlugin
{
    public class Connect : Extensibility.IDTExtensibility2, IDTCommandTarget
    {
        ...
        private const string GROUPDOCS_MENU_TOOLTIP = "Download and Run GroupDocs.Signature for .NET API Examples";

        private const string SIGNATURE_COMMAND_NAME = "GroupDocsSignatureVSPlugin";
        private const string SIGNATURE_COMMAND_CAPTION = "New GroupDocs.Signature Example Project";
        private const string SIGNATURE_COMMAND_TOOLTIP = "Download and Run GroupDocs.Signature for .NET API Examples";
            ...
            try
            {
                ...
                for (int iloop = 1; iloop <= toolsPopup.CommandBar.Controls.Count; iloop++)
                {
                    if (toolsPopup.CommandBar.Controls[iloop].Caption == GROUPDOCS_MENU_CAPTION)
                    {
                        ...
                        {
                            if (cmdbtn.Caption == SIGNATURE_COMMAND_CAPTION)
                            {
                                oBtn = cmdbtn;
                            }
                        }
                ...
                if (oBtn == null)
                {
                    ...
                    Command myCommand = commands.AddNamedCommand2(addInInstance, SIGNATURE_COMMAND_NAME,
                        SIGNATURE_COMMAND_CAPTION, SIGNATURE_COMMAND_TOOLTIP, false, Resources.pnglogosmall, ref contextGUIDS,
           ...
           ref object varOut, ref bool handled)
        {
            try
            {
                ...
                {
                    if (cmdName == addInInstance.ProgID + "." + SIGNATURE_COMMAND_NAME)
                    {
